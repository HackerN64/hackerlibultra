...
int __rmonListThreads(KKHeader *req) {
    ...
    __rmonSendReply(&reply->header,
                    sizeof(*reply) + sizeof(reply->objs.objects[0]) * (reply->objs.number - 1),
                    KK_TYPE_REPLY);
    ...
    switch (req->method) {
        ...
        case RMON_RSP:
            if (__rmonRCPrunning()) {
                ...
                if (__rmonGetBranchTarget(RMON_RSP, RMON_TID_RSP, (void *) ((u32) pc + SP_IMEM_START))
                        % 4
                    == 0) {
                    __rmonStepRCP();
                }
