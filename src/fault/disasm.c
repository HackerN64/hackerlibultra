...
char __mips_gpr[][4] = { "$r0", "$at", "$v0", "$v1", "$a0", "$a1", "$a2", "$a3", "$t0", "$t1", "$t2", "$t3",
...
static void c1_disasm(u32* ptr, char* out) {
    ...
    switch (*opn) {
        ...
        /* op rt, rs, imm */ case 'i':
            sprintf(out, "%-9s %s, %s, %d", opn + 1, rt, rs, (s16)op);
            break;
        /* op rt, imm */ case 'k':
            sprintf(out, "%-9s %s, %d", opn + 1, rt, (s16)op);
            break;
        /* op rt, imm(rs) */ case 'm':
            sprintf(out, "%-9s %s, %d(%s)", opn + 1, rt, (s16)op, rs);
            break;
        /* op fd, imm(rs) */ case 'n':
            sprintf(out, "%-9s %s, %d(%s)", opn + 1, __mips_fpreg[(op >> 16) & 0x1F], (s16)op, rs);
            break;
        /* op rd, rs, rt  */ case 'r':
            sprintf(out, "%-9s %s, %s, %s", opn + 1, rd, rs, rt);
            break;
        /* op rd, rs */ case 's':
            sprintf(out, "%-9s %s, %s", opn + 1, rd, rs);
            break;
        /* op rd, rt, sa  */ case 'e':
            sprintf(out, "%-9s %s, %s, %ld", opn + 1, rd, rt, (op >> 6) & 0x1F);
            break;
        /* op rs */ case 'w':
            sprintf(out, "%-9s %s", opn + 1, rs);
            break;
        /* op rd */ case 'c':
            sprintf(out, "%-9s %s", opn + 1, rd);
            break;
        /* op */ case 'z':
            sprintf(out, "%-9s", opn + 1);
            break;
        /* op fd, fs, ft */ case 'f':
            sprintf(out, "%-9s %s, %s, %s", opn + 1, rd, rs, rt);
            break;
        /* op rt, fs */ case 'g':
            sprintf(out, "%-9s %s, %s", opn + 1, rt, __mips_fpreg[(op >> 11) & 0x1F]);
            break;
        /* op rs, rt */ case 'h':
            sprintf(out, "%-9s %s, %s", opn + 1, rs, rt);
            break;
        /* op code20 */ case 'a':
            sprintf(out, "%-9s 0x%lx", opn + 1, (op >> 6) & 0xFFFFF);
            break;
        /* op rs, rt, code */ case 't':
            sprintf(out, "%-9s %s, %s, 0x%lx", opn + 1, rs, rt, (op >> 6) & 0x3FF);
            break;
        default:
            sprintf(out, "%-9s", opn + 1);
            break;
...
char* branch_insn_disasm(InsnData insn) {
    static char* insn_names[] = {
        [0b00001] = "bgez", [0b00011] = "bgezl", [0b10001] = "bgezal", [0b10011] = "bgezall",
        [0b00000] = "bltz", [0b00010] = "bltzl", [0b10000] = "bltzal", [0b10010] = "bltzall",
    };
        ...
        if (insn.i.opcode != 0 && insn.i.opcode == insn_db[i].opcode) {
            switch (insn_db[i].arbitraryParam) {
                case PARAM_SWAP_RS_IMM:
                    strp += sprintf(strp, "%-9s %s %s  %04X", insn_db[i].name, __mips_gpr[insn.i.rt],
                                    __mips_gpr[insn.i.rs], insn.i.immediate);
                    break;
                case PARAM_LUI:
                    strp += sprintf(strp, "%-9s %s %04X", insn_db[i].name, __mips_gpr[insn.i.rt], insn.i.immediate);
                    break;
                    break;
                case PARAM_JAL:
                ...
                case PARAM_FLOAT_RT:
                    strp += sprintf(strp, "%-9s %s, %04X (%s)", insn_db[i].name, __mips_fpreg[insn.i.rt],
                                    insn.i.immediate, __mips_gpr[insn.i.rs]);
                    break;
                case PARAM_NONE:
                    strp += sprintf(strp, "%-9s %s %04X (%s)", insn_db[i].name, __mips_gpr[insn.i.rt], insn.i.immediate,
            ...
            switch (insn_db[i].arbitraryParam) {
                case PARAM_BITSHIFT:
                    strp += sprintf(strp, "%-9s %s %s %04X", insn_db[i].name, __mips_gpr[insn.i.rdata.rd],
                                    __mips_gpr[insn.i.rt], insn.i.rdata.shift_amt);
                    break;
                case PARAM_SWAP_RS_RT:
                    strp += sprintf(strp, "%-9s %s %s %s", insn_db[i].name, __mips_gpr[insn.i.rdata.rd],
                                    __mips_gpr[insn.i.rt], __mips_gpr[insn.i.rs]);
                    break;
                case PARAM_MULT_MOVE:
                    strp += sprintf(strp, "%-9s %s", insn_db[i].name, __mips_gpr[insn.i.rdata.rd]);
                    break;
                case PARAM_EMUX:
                ...
                case PARAM_NONE:
                    strp += sprintf(strp, "%-9s %s %s %s", insn_db[i].name, __mips_gpr[insn.i.rdata.rd],
                                    __mips_gpr[insn.i.rs], __mips_gpr[insn.i.rt]);
                    break;
            }
            successful_print = 1;
            break;
